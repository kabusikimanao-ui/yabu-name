    ...
    if (opts.clickable) {
      ...
      if (opts.clickable === 'peek-select' && isMyTurn && roomView.currentPos === 0 && tl && !tl.evidenceSeen && !isPeeked) {
        ...
        card.onclick = () => {
          ...
          // 状態更新は呼び出し元で render を呼ぶ前提、またはここで直接呼ぶ
          const { render } = require('./ui-render.js'); // 循環参照避けるため、window.render やコールバックを使うのが安全だが、簡易的に再描画トリガーを呼ぶ
          // 実際には app.js 側で管理する render 関数を呼ぶ必要がありますが、ここでは簡略化のため window.renderApp などを呼ぶか、
          // ui-render.js の render をインポートします（ui-render.js が game-view を呼ばない限りはOK）
        };
